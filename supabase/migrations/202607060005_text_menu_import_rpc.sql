...
begin
  ...
  if exists (
    ...
    where length(coalesce(item_value->>'name', '')) > 160
       or length(coalesce(item_value->>'shortName', '')) > 80
       or length(coalesce(item_value->>'description', '')) > 280
  ) then
  ...
  if exists (
    ...
    cross join lateral jsonb_array_elements(coalesce(sections.section_value->'items', '[]'::jsonb)) with ordinality as items(item_value, item_ordinal)
    group by
      coalesce(sections.section_value->>'id', 'section-' || (section_ordinal - 1)::text),
      coalesce(item_value->>'id', 'item-' || (section_ordinal - 1)::text || '-' || (item_ordinal - 1)::text)
    having count(*) > 1
  ) then
  ...
  on conflict (org_id, canteen_id, menu_date)
  do update set
  ...
  loop
    ...
    for menu_item in
      select value
      from jsonb_array_elements(coalesce(section_item->'items', '[]'::jsonb))
    loop
  ...
  set current_version_id = version_row.id,
      status = 'draft'
  where org_id = canteen_row.org_id
    and id = menu_row.id
  returning * into menu_row;
