-- Re-apply the safe live deck RPC after immutable template version hardening.
-- Production may already have older migration history where version 1 templates
-- were updated with ON CONFLICT DO UPDATE, which now correctly fails.
...
begin
  ...
  on conflict (org_id, template_id, version) do nothing;

  select tv.id
  into daily_template_version_id
  from public.template_versions as tv
  where tv.org_id = version_row.org_id
    and tv.template_id = daily_template_id
    and tv.version = 1;
  ...
  select tv.id
  into special_template_version_id
  from public.template_versions as tv
  where tv.org_id = version_row.org_id
    and tv.template_id = special_template_id
    and tv.version = 1;
  ...
  select tv.id
  into legend_template_version_id
  from public.template_versions as tv
  where tv.org_id = version_row.org_id
    and tv.template_id = legend_template_id
    and tv.version = 1;
